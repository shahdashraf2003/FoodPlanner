...
public class MealFilterByDataSource  {
    /*private static final String TAG = "MealFilterByDataSource";
    private Services mealFilterbyService;

    public MealFilterByDataSource() {

        mealFilterbyService = Network.getInstance().services;
    }

    public void getRandomMeal(MealFilterByResponse callback){
        mealFilterbyService.getRandomMeal().enqueue(new Callback<MealResponse>() {
            @Override
